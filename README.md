...
Bu bölümde, SQL enjeksiyonunun ne olduğunu açıklayacak, bazı yaygın örnekleri tanımlayacak, çeşitli türde SQL enjeksiyon açıklarını nasıl bulunacağını ve sömürüleceğini açıklayacak ve SQL enjeksiyonunu önlemenin nasıl yapıldığını özetleyeceğiz.

#### SQL enjeksiyonu (SQLi) nedir?

SQL enjeksiyonu, bir saldırganın bir uygulamanın veritabanına yaptığı sorgulara müdahale etmesine olanak tanıyan bir web güvenlik açığıdır. Genellikle bir saldırganın normalde alamayacağı verileri görüntülemesine izin verir. Bu, diğer kullanıcılara ait verileri veya uygulamanın erişebildiği diğer verileri içerebilir. Bir saldırgan birçok durumda bu verileri değiştirebilir veya silebilir, bu da uygulamanın içeriğinde veya davranışında kalıcı değişikliklere neden olabilir.

Bazı durumlarda, bir saldırgan SQL enjeksiyon saldırısını kullanarak altta yatan sunucuyu veya diğer arka uç altyapıyı tehlikeye atabilir veya reddedilmiş hizmet saldırısı gerçekleştirebilir.

<p align="center"> 
<img src="/Image/sql-injection.svg">
</p>

|    SQL Enjeksiyon Türü        | Açıklama                     
|----------------|-------------------------------|
|In-band SQLi (Klasik SQLi)|In-band SQL enjeksiyonu, SQL enjeksiyon saldırılarının en yaygın ve kolay sömürülebilen türüdür. In-band SQL enjeksiyonu, bir saldırganın saldırıyı başlatmak ve sonuçları toplamak için aynı iletişim kanalını kullanabilmesi durumunda gerçekleşir. In-band SQL enjeksiyonunun en yaygın iki türü hata tabanlı SQLi ve birleştirme tabanlı SQLi'dir. |    
|Hata tabanlı SQLi          |Hata tabanlı SQL enjeksiyonu, veritabanı sunucusu tarafından atılan hata mesajlarını kullanarak veritabanının yapısı hakkında bilgi elde etmeye dayanan bir in-band SQL enjeksiyon tekniğidir. Bazı durumlarda, hata tabanlı SQL enjeksiyonu tek başına bir saldırganın tüm veritabanını sıralaması için yeterli olabilir.| 
|Birleştirme tabanlı SQLi         |Birleştirme tabanlı SQL enjeksiyonu, UNION SQL operatörünü kullanarak iki veya daha fazla SELECT ifadesinin sonuçlarını tek bir sonuç halinde birleştirir ve bunu HTTP yanıtının bir parçası

 olarak döndürür.| 
|İnferential SQLi (Kör SQLi)|In-band SQL enjeksiyonunun aksine, inferential SQL enjeksiyonu bir saldırganın sömürmek için daha uzun süre gerektirebilir, ancak diğer SQL enjeksiyon türleri kadar tehlikelidir. Kör SQL enjeksiyon saldırısında, aslında hiçbir veri web uygulaması aracılığıyla aktarılmaz ve saldırgan saldırının sonucunu in-band göremez (bu nedenle bu tür saldırılara genellikle "kör SQL enjeksiyonu saldırıları" denir). Bunun yerine, bir saldırgan, yükler göndererek, web uygulamasının yanıtını ve veritabanı sunucusunun davranışını gözlemleyerek veritabanının yapısını yeniden oluşturabilir. Kör SQL enjeksiyonun iki türü vardır: Kör mantık tabanlı SQLi ve Kör zaman tabanlı SQLi.|
|Mantık tabanlı (içerik tabanlı) Kör SQLi |Mantık tabanlı SQL enjeksiyonu, veritabanına bir SQL sorgusu göndererek uygulamanın sorgunun TRUE veya FALSE sonucuna bağlı olarak farklı bir sonuç döndürmesine dayanan bir inferential SQL enjeksiyon tekniğidir. Sonuca bağlı olarak, HTTP yanıtındaki içerik değişecek veya aynı kalacaktır. Bu, saldırganın veritabanından hiçbir veri döndürülmemesine rağmen kullandığı yükün TRUE veya FALSE döndürdüğünü çıkarabilmesine olanak tanır.|
|Zaman tabanlı Kör SQLi |Zaman tabanlı SQL enjeksiyonu, veritabanına bir SQL sorgusu göndererek veritabanının yanıt vermeden belirli bir süre (saniye cinsinden) beklemesini sağlayan bir inferential SQL enjeksiyon tekniğidir. Yanıt süresi, saldırganın sorgunun sonucunun TRUE veya FALSE olduğunu belirtecektir. Sonuca bağlı olarak, bir HTTP yanıtı gecikmeli olarak veya hemen dönecektir. Bu, saldırganın veritabanından hiçbir veri döndürülmemesine rağmen kullandığı yükün TRUE veya FALSE döndürdüğünü çıkarabilmesine olanak tanır.|
|Out-of-band SQLi|Out-of-band SQL enjeksiyonu pek yaygın değildir, çoğunlukla web uygulaması tarafından kullanılan veritabanı sunucusunda etkinleştirilen özelliklere bağlıdır. Out-of-band SQL enjeksiyonu, bir saldırganın saldırıyı başlatmak ve sonuçları toplamak için aynı kan

alı kullanamaması durumunda gerçekleşir. Out-of-band teknikler, sunucu yanıtlarının çok kararlı olmaması durumunda (bu da bir inferential zaman tabanlı saldırının güvenilir olmasını engeller) bir saldırgana inferential zaman tabanlı tekniklere alternatif bir seçenek sunar.|
|Ses Tabanlı SQL Enjeksiyonu|Sesli komutlarla veritabanına erişim sağlayan uygulamalarda uygulanabilen bir SQL enjeksiyon saldırı yöntemidir. Bir saldırgan, sesli komutlarla sql sorguları göndererek veritabanından bilgi çekebilir. |
